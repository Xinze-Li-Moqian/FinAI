...
def main():
    ...
    source = root / 'materials/APS1053/03_Generative_AI_Assignment/Youtube_AI_students/transcripts_ids_upto_20260514'
    entries = []
    if args.metadata:
    ...
    download_log = out / 'download_log.json'
    downloads = json.loads(download_log.read_text()) if download_log.exists() else {}
    sources = sorted(source.glob('*.txt')) + sorted((out / 'new_transcripts').glob('*.txt'))
    for file in sources:
        ...
        try:
            ...
            source_path = 'courses/APS1053/assignments/generative-ai/part1/new_transcripts/' + file.name
    ...
    write_csv(out / 'transcript_manifest.csv', records, list(records[0]))
    if args.materialize:
    ...
    write_csv(out / 'youtube_channel_videos.csv',
    ...
    (out / 'inventory.json').write_text(json.dumps(status, indent=2) + '\n')
