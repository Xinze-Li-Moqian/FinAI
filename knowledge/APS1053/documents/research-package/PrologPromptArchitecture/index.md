...
来源：<../../../../../materials/APS1053/07_Prompt_Pipeline/DeepSeek_revised2_by_chatgpt/Document/PromptProlog/PrologPromptArchitecture.docx>

自动转换的阅读副本；请以原稿核对布局和公式。

# 

Claude: This paper is based on 3 sources:
...
The impulse behind this article was modest: the conventional eight-part prompt anatomy feels adequate for most prompts, and it is. What the map-reduce-summarisation pipeline exposed is where it stops being adequate — not at the level of any single instruction, but at the level of the whole design. Once a prompt must coordinate a many-to-many domain model, a closed vocabulary of argumentative roles, and a division of labor between two different reasoning systems across five dependent phases, "Instructions" and "Context" stop functioning as categories and start functioning as a junk drawer. Prolog does not resolve this by supplying better syntax. It resolves it by refusing, from the outset, to let facts, rules, and queries collapse into one undifferentiated container — and by reserving one construct, the directive, whose entire purpose is to speak about the process rather than about the world.

That is the distinction this article kept returning to in progressively more concrete form: Object/Domain versus Meta/Process, Facts versus Directives, a Context that behaves like an Entity-Relationship diagram versus Instructions that behave like a control program. The value of the distinction is not that it is philosophically tidy; it is that it is checkable. The clearest evidence of that is the article's last worked example. Least Agency began as a single sentence — Python validates the model's output before accepting it. Insisting on tracing that sentence to its Object side and its Meta side separately did not just clarify the sentence; it led somewhere concrete: an actual Entity-Relationship table (Table A — canonical Central Claims, canonical Themes, persistent CC × Theme pairs, and their cardinalities) and an actual file, integrity_report.json, that audits a real pipeline's real output against exactly that table, phase by phase.

That is the claim underneath the Prolog analogy, once the analogy is followed all the way down: a directive that cannot be traced to a checkable object-level structure is not yet a directive — it is still an aspiration written in the imperative mood. A complex prompt is not a longer simple prompt; it is a specification for a system, and a specification for a system needs the discipline a database schema or a logic program has always demanded: know what you are asserting about the world, know what you are commanding the process to do, and never let a sentence of the second kind quietly pass itself off as the first. The six-part anatomy tells you what to write. The object/meta split tells you what you actually said.
